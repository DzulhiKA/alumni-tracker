...
export default async function AdminPage() {
  ...
  // Stats
  const statusGroups = (allAlumni || []).reduce((acc, a) => {
  ...
  const avgCompleteness = completenessData?.length
    ? Math.round(completenessData.reduce((s, c) => s + (c.completeness_percent || 0), 0) / completenessData.length)
    : 0
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-6">
          <div>
            ...
            <p className="text-slate-500 mt-0.5">Kelola seluruh data alumni</p>
          </div>

          {/* Stats */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="card p-5">
              <p className="text-3xl font-bold text-blue-700">{allAlumni?.length ?? 0}</p>
              <p className="text-slate-500 text-sm mt-1">Total Alumni</p>
            </div>
            <div className="card p-5">
              <p className="text-3xl font-bold text-emerald-600">{statusGroups['employed'] ?? 0}</p>
              <p className="text-slate-500 text-sm mt-1">Bekerja</p>
            </div>
            <div className="card p-5">
              <p className="text-3xl font-bold text-blue-600">{statusGroups['studying'] ?? 0}</p>
              <p className="text-slate-500 text-sm mt-1">Studi Lanjut</p>
            </div>
            <div className="card p-5">
              <p className="text-3xl font-bold text-amber-600">{avgCompleteness}%</p>
              <p className="text-slate-500 text-sm mt-1">Rata-rata Kelengkapan</p>
            </div>
          </div>
